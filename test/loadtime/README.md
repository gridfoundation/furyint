...
transaction load against furyint and measuring their resulting latency.
...
transactions and broadcasts them to Furyint.
...
Below is a basic invocation for generating load against a Furyint websocket running
on `localhost:25567`
...
Below is a basic invocation of the report tool with a data directory under `/home/test/.furyint/data/`

```bash
./build/report --data-dir ~/.furyint/data
```
...
```bash
./build/report --data-dir ~/.furyint/data --csv out.csv
```
