...
import (
	...
	"github.com/gridfoundation/cosmosclient/cosmosclient"
...
// Config for the GridironLayerClient
type Config struct {
...
func (dlc *LayerClient) Init(config []byte, pubsub *pubsub.Server, logger log.Logger, options ...settlement.Option) error {
	GridironCosmosClient, err := newGridironHubClient(config, pubsub, logger)
	if err != nil {
		return err
	}
	baseOptions := []settlement.Option{
		settlement.WithHubClient(GridironCosmosClient),
	}
	if options == nil {
		options = baseOptions
	} else {
...
// PostBatchResp is the response after posting a batch to the Gridiron Hub.
...
// HubClient is the client for the Gridiron Hub.
...
func newGridironHubClient(config []byte, pubsub *pubsub.Server, logger log.Logger, options ...Option) (*HubClient, error) {
...
// PostBatch posts a batch to the Gridiron Hub.
...
// GetLatestBatch returns the latest batch from the Gridiron Hub.
...
// GetBatchAtIndex returns the batch at the given index from the Gridiron Hub.
...
func (d *HubClient) eventHandler() {
	...
	eventsChannel, err := d.client.SubscribeToEvents(context.Background(), "gridiron-client", fmt.Sprintf(eventStateUpdate, d.config.RollappID))
	if err != nil {
